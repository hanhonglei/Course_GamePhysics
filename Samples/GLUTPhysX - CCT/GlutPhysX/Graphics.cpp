...
void renderActors(PxCloth** actors, const PxU32 numActors, bool shadows)
{
...
void RenderPhysXScene()
{
	...
	nbActors = scene->getNbActors(PxActorTypeFlag::eCLOTH);
	if (nbActors)
	{
		glDisable(GL_LIGHTING);
		...
		scene->getActors(PxActorTypeFlag::eCLOTH, reinterpret_cast<PxActor**>(&actors[0]), nbActors);

		for (int ii = 0; ii < nbActors; ii++)
		{
			PxCloth *cloth = (reinterpret_cast<PxCloth**>(&actors[0]))[ii];
			PxClothParticleData* data = cloth->lockParticleData(PxDataAccessFlag::eREADABLE);
			PxMat44 shapePose = cloth->getGlobalPose();
			// render object
			glPushMatrix();
			glMultMatrixf(reinterpret_cast<const float*>(&shapePose));
			glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
			...
			for (PxU32 i = 0, n = cloth->getNbParticles(); i < n; ++i)
				glVertex3f(data->previousParticles[i].pos.x, data->previousParticles[i].pos.y, data->previousParticles[i].pos.z);
			glEnd();
			for (PxU32 i = 0, n = cloth->getNbParticles(); i < n; ++i)
			{
				glPushMatrix();
				glTranslatef(data->previousParticles[i].pos.x, data->previousParticles[i].pos.y, data->previousParticles[i].pos.z);
